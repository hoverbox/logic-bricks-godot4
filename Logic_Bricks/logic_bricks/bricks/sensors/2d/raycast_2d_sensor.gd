...
## Type the RayCast2D node name
## Supports group filtering — only detect objects in specific groups
## The RayCast2D's direction and length are configured in the inspector
...
func get_tooltip_definitions() -> Dictionary:
	return {
		"_description": "Detects along a RayCast2D. This can be used for objects, groups, walls, ground, or ledges.\nThe RayCast2D's direction, length, and collision mask are set on the RayCast2D node itself.",
...
func generate_code(node: Node, chain_name: String) -> Dictionary:
	var raycast_node_name = str(properties.get("raycast_node_name", "RayCast2D")).strip_edges()
	...
	member_vars.append("var %s: RayCast2D = null" % raycast_var)
	...
	code_lines.append("\tif _found_node_%s is RayCast2D:" % chain_name)
	code_lines.append("\t\t" + raycast_var + " = _found_node_%s" % chain_name)
	code_lines.append("\telif _found_node_%s:" % chain_name)
	code_lines.append("\t\tpush_warning(\"Raycast Sensor: node '\" + str(_node_name_%s) + \"' is not a RayCast2D\")" % chain_name)
	...
	code_lines.append("else:")
	code_lines.append("\tsensor_active = false")
	code_lines.append("\tpush_warning(\"Raycast Sensor: No RayCast2D assigned to '%s'\")" % raycast_var)
