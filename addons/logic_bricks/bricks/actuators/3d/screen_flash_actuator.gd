...
func generate_code(node: Node, chain_name: String) -> Dictionary:
	...
	code_lines.append("\t# Check the node's own scene root first (handles subscenes where the")
	code_lines.append("\t# CanvasLayer lives inside the instanced scene), then fall back to")
	code_lines.append("\t# current_scene (handles the case it was placed in the parent scene).")
	...
	code_lines.append("\tfor _flash_search_root_%s in [_flash_own_root_%s, get_tree().current_scene]:" % [chain_name, chain_name])
	code_lines.append("\t\tif not is_instance_valid(_flash_search_root_%s): continue" % chain_name)
	code_lines.append("\t\tfor _flash_cl_%s in _flash_search_root_%s.get_children():" % [chain_name, chain_name])
	code_lines.append("\t\t\tif _flash_cl_%s is CanvasLayer and _flash_cl_%s.name == \"%s\":" % [chain_name, chain_name, layer_name])
	code_lines.append("\t\t\t\t%s = _flash_cl_%s.get_node_or_null(\"ColorRect\") as ColorRect" % [flash_var, chain_name])
	code_lines.append("\t\t\t\tbreak")
	code_lines.append("\t\tif is_instance_valid(%s): break" % flash_var)
