...
func generate_code(node: Node, chain_name: String) -> Dictionary:
	...
	member_vars.append("var _shake_offset: Vector3 = Vector3.ZERO")

	# Helper method
	member_vars.append("")
	member_vars.append("func _process_screen_shake(_delta: float) -> void:")
	member_vars.append("\tif not %s: return" % cam_var)
	member_vars.append("\t%s.global_position -= _shake_offset" % cam_var)
	member_vars.append("\t%s.rotation_degrees.z -= _shake_offset.z * 100.0" % cam_var)
	...
	member_vars.append("\t\t_shake_offset = Vector3(")
	member_vars.append("\t\t\t_shake_max_offset * _shake_amount * _shake_noise.get_noise_2d(_shake_time * _shake_noise_speed, 0.0),")
	member_vars.append("\t\t\t_shake_max_offset * _shake_amount * _shake_noise.get_noise_2d(0.0, _shake_time * _shake_noise_speed),")
	member_vars.append("\t\t\t_shake_max_rotation * 0.01 * _shake_amount * _shake_noise.get_noise_2d(_shake_time * _shake_noise_speed, _shake_time * _shake_noise_speed)")
	member_vars.append("\t\t)")
	member_vars.append("\t\t%s.global_position += _shake_offset" % cam_var)
	member_vars.append("\t\t%s.rotation_degrees.z += _shake_offset.z * 100.0" % cam_var)
	member_vars.append("\t\t_shake_trauma = maxf(_shake_trauma - _shake_decay * _delta, 0.0)")
	member_vars.append("\telse:")
	member_vars.append("\t\t_shake_offset = Vector3.ZERO")
