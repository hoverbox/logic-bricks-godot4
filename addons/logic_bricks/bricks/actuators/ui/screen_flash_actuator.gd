...
func get_brick_info() -> Dictionary:
	return {
		"class": "UIScreenFlashActuator",
		"name": "Screen Flash",
		"type": "actuator",
		"category": "UI",
		"description": "Flashes a full-screen ColorRect overlay.",
		"menu_order": 150,
		"domain": "ui"
	}
...
func generate_code(node: Node, chain_name: String) -> Dictionary:
	...
	code_lines.append("else:")
	code_lines.append("\t# Standard: resolve or create a full-screen flash layer at runtime.")
	code_lines.append("\t# This makes Screen Flash work from instanced player scenes too, even")
	code_lines.append("\t# if the editor-created helper nodes were not saved into this scene.")
	code_lines.append("\tvar %s: ColorRect = null" % flash_var)
	code_lines.append("\tvar _flash_layer_%s: CanvasLayer = null" % chain_name)
	code_lines.append("\tvar _flash_roots_%s: Array = []" % chain_name)
	code_lines.append("\tvar _flash_own_root_%s = get_owner() if is_instance_valid(get_owner()) else self" % chain_name)
	code_lines.append("\tif is_instance_valid(_flash_own_root_%s): _flash_roots_%s.append(_flash_own_root_%s)" % [chain_name, chain_name, chain_name])
	code_lines.append("\tif is_instance_valid(get_tree().current_scene) and get_tree().current_scene not in _flash_roots_%s: _flash_roots_%s.append(get_tree().current_scene)" % [chain_name, chain_name])
	code_lines.append("\t# Search recursively because helper nodes may live inside an instanced scene root.")
	code_lines.append("\tfor _flash_search_root_%s in _flash_roots_%s:" % [chain_name, chain_name])
	code_lines.append("\t\tfor _flash_node_%s in _flash_search_root_%s.find_children(\"%s\", \"CanvasLayer\", true, false):" % [chain_name, chain_name, layer_name])
	code_lines.append("\t\t\t_flash_layer_%s = _flash_node_%s as CanvasLayer" % [chain_name, chain_name])
	code_lines.append("\t\t\tbreak")
	code_lines.append("\t\tif is_instance_valid(_flash_layer_%s): break" % chain_name)
	code_lines.append("\t# Runtime self-heal: create the layer/rect if Apply Code did not place them where this instance can see them.")
	code_lines.append("\tif not is_instance_valid(_flash_layer_%s):" % chain_name)
	code_lines.append("\t\tvar _flash_parent_%s: Node = get_tree().current_scene if is_instance_valid(get_tree().current_scene) else _flash_own_root_%s" % [chain_name, chain_name])
	code_lines.append("\t\tif is_instance_valid(_flash_parent_%s):" % chain_name)
	code_lines.append("\t\t\t_flash_layer_%s = CanvasLayer.new()" % chain_name)
	code_lines.append("\t\t\t_flash_layer_%s.name = \"%s\"" % [chain_name, layer_name])
	code_lines.append("\t\t\t_flash_layer_%s.layer = 128" % chain_name)
	code_lines.append("\t\t\t_flash_parent_%s.add_child(_flash_layer_%s)" % [chain_name, chain_name])
	code_lines.append("\tif is_instance_valid(_flash_layer_%s):" % chain_name)
	code_lines.append("\t\t%s = _flash_layer_%s.get_node_or_null(\"ColorRect\") as ColorRect" % [flash_var, chain_name])
	code_lines.append("\t\tif not is_instance_valid(%s):" % flash_var)
	code_lines.append("\t\t\t%s = ColorRect.new()" % flash_var)
	code_lines.append("\t\t\t%s.name = \"ColorRect\"" % flash_var)
	code_lines.append("\t\t\t_flash_layer_%s.add_child(%s)" % [chain_name, flash_var])
	...
	code_lines.append("\t\t%s.mouse_filter  = Control.MOUSE_FILTER_IGNORE" % flash_var)
	code_lines.append("\t\tvar %s = create_tween()" % tw_var)
	...
	code_lines.append("\telse:")
	code_lines.append("\t\tpush_warning(\"Screen Flash Actuator: could not create or find '%s/ColorRect'\")" % layer_name)
