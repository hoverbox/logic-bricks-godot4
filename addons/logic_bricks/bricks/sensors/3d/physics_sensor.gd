...
## Physics Sensor - Detects CharacterBody3D contact state.
...
func get_tooltip_definitions() -> Dictionary:
	return {
		"_description": "Checks CharacterBody3D collision state: on floor, on wall, or on ceiling.\nUse this for jump checks, wall checks, ceiling bumps, and platformer logic.",
...
func generate_code(node: Node, chain_name: String) -> Dictionary:
	...
	code_lines.append("\t\tpush_warning(\"Physics Sensor: This node does not support %s(). Use it on a CharacterBody3D/CharacterBody2D.\")" % method_name)
