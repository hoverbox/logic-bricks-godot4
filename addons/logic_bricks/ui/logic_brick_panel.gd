...
func _open_search_popup(screen_pos: Vector2, initial_text: String = "") -> void:
    if not is_instance_valid(_search_popup):
        _build_search_popup()
    
    # Seed the search field
    if is_instance_valid(_search_edit_ref):
        _search_edit_ref.text = initial_text
        _search_edit_ref.caret_column = initial_text.length()
    
    _search_update_results(initial_text)
    
    # Position on screen, keeping in bounds
    var screen_size = DisplayServer.screen_get_size()
    # Force a size recalc before reading it
    _search_popup.reset_size()
    var pop_size = Vector2(_search_popup.size)
    if pop_size == Vector2.ZERO:
        pop_size = Vector2(456, 300)  # Fallback before first frame
    var pos = screen_pos
    ...
    _search_popup.position = Vector2i(pos)
    _search_popup.popup()
    
    if is_instance_valid(_search_edit_ref):
        _search_edit_ref.call_deferred("grab_focus")


func _build_search_popup() -> void:
    _search_popup = PopupPanel.new()
    _search_popup.name = "BrickSearchPopup"
    _search_popup.exclusive = false
    _search_popup.unresizable = true
    
    var vbox = VBoxContainer.new()
    vbox.name = "VBox"
    vbox.add_theme_constant_override("separation", 4)
    vbox.custom_minimum_size = Vector2(440, 0)
    _search_popup.add_child(vbox)
    ...
    _search_no_results_ref = no_results
    
    add_child(_search_popup)


const _SEARCH_BRICKS: Array = [
...
const _SEARCH_TYPE_COLORS: Dictionary = {
    "sensor":     Color(0.2, 0.55, 0.2,  0.9),
    "controller": Color(0.2, 0.3,  0.75, 0.9),
    "actuator":   Color(0.6, 0.3,  0.1,  0.9),
}

var _search_selected_index: int = -1
var _search_result_buttons: Array = []
var _search_edit_ref: LineEdit = null
var _search_results_ref: VBoxContainer = null
var _search_scroll_ref: ScrollContainer = null
var _search_no_results_ref: Label = null
...
func _search_on_key(event: InputEvent) -> void:
    if not event is InputEventKey or not event.pressed:
        return
    var n = _search_result_buttons.size()
    if n == 0:
    ...
    match event.keycode:
        KEY_DOWN:
            _search_set_selected((_search_selected_index + 1) % n)
            if is_instance_valid(_search_edit_ref): _search_edit_ref.accept_event()
        KEY_UP:
            _search_set_selected((_search_selected_index - 1 + n) % n)
            if is_instance_valid(_search_edit_ref): _search_edit_ref.accept_event()
        KEY_ENTER, KEY_KP_ENTER:
            if _search_selected_index >= 0 and _search_selected_index < n:
                _search_result_buttons[_search_selected_index].pressed.emit()
            if is_instance_valid(_search_edit_ref): _search_edit_ref.accept_event()
        KEY_ESCAPE:
            _search_popup.hide()
            if is_instance_valid(_search_edit_ref): _search_edit_ref.accept_event()
...
func _search_update_results(query: String) -> void:
    if not is_instance_valid(_search_popup):
        return
    if not is_instance_valid(_search_results_ref):
        return
    
    for child in _search_results_ref.get_children():
        child.queue_free()
    _search_result_buttons.clear()
    _search_selected_index = -1
    
    var scored: Array = []
    for brick in _SEARCH_BRICKS:
    ...
    for i in range(show_count):
        ...
        _search_results_ref.add_child(btn)
        _search_result_buttons.append(btn)
    
    if is_instance_valid(_search_no_results_ref):
        _search_no_results_ref.visible = (show_count == 0 and not query.is_empty())
    
    if is_instance_valid(_search_scroll_ref):
        _search_scroll_ref.custom_minimum_size = Vector2(440, mini(show_count, 12) * 36 + 4)
    
    _search_popup.reset_size()
    
    if _search_result_buttons.size() > 0:
...
func _search_make_button(brick: Array, index: int) -> Button:
    ...
    sbox.bg_color = _SEARCH_TYPE_COLORS.get(brick_type, Color.GRAY)
...
func _search_set_selected(index: int) -> void:
    if _search_selected_index >= 0 and _search_selected_index < _search_result_buttons.size():
        _search_result_buttons[_search_selected_index].remove_theme_stylebox_override("normal")
    _search_selected_index = index
    if index >= 0 and index < _search_result_buttons.size():
        ...
        _search_result_buttons[index].add_theme_stylebox_override("normal", hl)
        if is_instance_valid(_search_scroll_ref):
            _search_scroll_ref.ensure_control_visible(_search_result_buttons[index])
...
func _search_score(query: String, name: String, type: String) -> float:
    ...
    var y = type.to_lower()
    if t == q: return 100.0
    if t.begins_with(q): return 80.0
    if t.contains(q): return 60.0
    if y.contains(q): return 30.0
...
func _search_word_score(qw: String, tw: String) -> float:
    if tw == qw: return 40.0
    if tw.begins_with(qw): return 30.0
    if tw.contains(qw): return 20.0
...
func _on_search_brick_selected(brick_type: String, brick_class: String) -> void:
    if is_instance_valid(_search_popup):
        _search_popup.hide()
